@php
    $pageName = 'Working Hours';

    $tableHead = ['Full Name' , 'Machine Name' , 'Opening Hours' , 'Closing Hours' , 'Working Hours' ,  'Entry Date' , 'Action'  ];
...
@section('content')
    ...
    <x-reusables.app-header pageName="{{ $pageName }}" :createButton="true" module="machine-working-hours" modulePlaceholder="Working Hours" />
            ...
            <div class="block-content block-content-full">
                ...
                <form class="row g-3 align-items-center" >
                    <div class="col">
                        ...
                        <select  class="form-control" name="user_id">
                            ...
                            @foreach ($users as $user)
                                <option value="{{$user->id}}" {{$user->id == (request()->user_id ?? 0)? 'selected' : '' }} >{{ucfirst($user->name)}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col">
                    ...
                    <div class="col">
                        ...
                        <a class="btn btn-secondary" href="{{route('machine-working-hours-listing')}}">Reset</a>
                    </div>
                </form>
                        ...
                        <tbody>
                                ...
                                @foreach ($listingData as $data)
                                    ...
                                    <tr>
                                        ...
                                        <td class="whitespace-nowrap ">
                                            {{ $data->opening_hours ?? 0}}
                                        </td>

                                        <td class="whitespace-nowrap ">
                                            {{ $data->closing_hours ?? 0 }}
                                        </td>
                                        ...
                                        <td class="whitespace-nowrap">
                                            <x-reusables.action-buttons :id="encrypt($data->id)" module="machine-working-hours"
                                                :name="$data->machinery->machinery_name ?? ''" />
                                        </td>

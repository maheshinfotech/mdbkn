...
class FrontController extends Controller
{
    ...
    public function getMachinery(){
        ...
        if(  in_array( auth('sanctum')->user()->role_id , [ config('app.admin_role') , config('app.manager_role') ] ) ){
    ...
    public function setFuelConsumption(){
        ...
        $machinery_association_count = in_array( auth('sanctum')->user()->role_id , [ config('app.admin_role') , config('app.manager_role') ]) 
                                            ? 1 : auth('sanctum')->user()->machinery()->where("machinery.id" , $machine_id)->count();
        
        if( !$machinery_association_count ){
    ...
    public function massInsertFuelConsumption(){
        ...
        foreach (request()->records as $key => $record) {
            ...
            $machinery_association_count =  in_array( auth('sanctum')->user()->role_id , 
                                                    [ config('app.admin_role') , config('app.manager_role') ]) 
                                            ? 1 : auth('sanctum')->user()->machinery()->where("machinery.id" , $machine_id)->count();

            if( !$machinery_association_count ){
                
                array_push($rejected_array , [
                                                'user_id'       => auth('sanctum')->user()->id , 
                                                'rejected_data' => json_encode($record) , 
                                                'reason'        => 'Unauthecticated User',
        ...
        DB::table('rejected_fuel_reading')->insert($rejected_array);
    ...
    public function machineAccess(){
        
        request()->validate([
            'machine_id' => 'required'
        ]);

        $machine = decrypt(request()->machine_id);

        $res = in_array( auth('sanctum')->user()->role_id , [ config('app.admin_role') , config('app.manager_role') ]) 
                        ? 1 : 
               auth('sanctum')->user()->machinery()->where("machinery.id" , $machine)->count();

        return $this->generateResponse( $res ? true : false , 
                                        $res ? config("app.authorized") : config('app.unauthorized') 
                                    );
    }
